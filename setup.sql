...
CREATE TABLE IF NOT EXISTS OrderItems(user_ID INT UNSIGNED, order_ID INT UNSIGNED,
item INT UNSIGNED, quantity INT UNSIGNED,  price DECIMAL (10,2),
FOREIGN KEY (order_ID) REFERENCES Orders(ID) ON UPDATE CASCADE ON DELETE CASCADE,
FOREIGN KEY (user_ID) REFERENCES Users(ID) ON UPDATE CASCADE ON DELETE CASCADE,
FOREIGN KEY (item) REFERENCES Products(ID) ON UPDATE CASCADE ON DELETE SET NULL,
...
FOREIGN KEY (ID) REFERENCES Orders(ID) ON UPDATE CASCADE ON DELETE CASCADE, PRIMARY KEY (ID, addr_type));
...
CREATE TRIGGER product_deletion BEFORE DELETE ON Products FOR EACH ROW
  BEGIN
    DECLARE price DECIMAL(10,2);
    DECLARE idExists BOOLEAN;

    SELECT 1 INTO @idExists FROM OrderItems oi WHERE oi.item = OLD.ID;
    -- Eh. Primary key fail. Ooops. Better fix
  END;
